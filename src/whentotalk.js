 ...
 *	Interval object.
 */
var Interval = function(start, end, offset, zoneObj){
...
Interval.prototype.getUTCIntervals = function() {
	...
	if (start > end){
		// split
		returnIntervals.push(new Interval(start, 24, this.offset, this.zoneObj));
		returnIntervals.push(new Interval(0, end, this.offset, this.zoneObj));
	} else {
		// regular
		returnIntervals.push(new Interval(start, end, this.offset, this.zoneObj));
	}
...
Person.prototype.readInterval = function() {
	if (this.cityIdx < 0) {
		...
		var zone = moment.tz.zone(cities[this.cityIdx]);
		// Division by -60.0 because offsets in moment-timezone package are differently coded.
		this.interval 	= new Interval(parseInt(range[0]), parseInt(range[1]), (zone.offset(currentTime)/-60.0), zone);
	}
}
...
Person.prototype.displayResult = function(intervals) {
	...
	for (var i = 0; i < intervals.length; i++) {
		var zone = moment.tz.zone(cities[this.cityIdx]);
		var newInterval = new Interval((intervals[i].start + offset + 24)%24, (intervals[i].end + offset + 24)%24, 0, zone);
...
function findMinMax(intervals){
	var result = new Interval(-1, -1, 0, null);
	var min = 0-1;
	var max = 24+1;
